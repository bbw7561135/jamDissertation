\chapter{The 1\st \Fermi -LAT Supernova Remnant Catalog \jamie{Change chapter title to better reflect the work I've done}}
\label{chap:snrCat}

\section{Introduction}\label{snrCat:Intro}
One  of the primary science goals of \Fermi~is to   resolve the \gam~sky and uncover the nature of the unidentified sources detected by \egret. By the end of the \egret~era, the telescope had detected 271 sources above 100\mev \jamie{energy range of egret was 30 \mev to 20 \gev, but in the 3EG paper they don't say whether this was the actual max energy used. they do make maps $>1\gev $}, 170 of which had no clear multiwavelength  counterpart, and 81 of those unidentified lying within \blat of the Galactic plane \citep{Hartman99}. The main hindrances to source identification were the numerous potential source counterparts (the \egret ~ \psf~ was energy dependent, with a 67\% containment radius of ${\rm \sim 6 ^\circ}$ at 100 \mev~and smaller for higher energies) and the large \egret~error boxes \citep{Hartman99}. 
Figure \ref{fig:3EGSky} shows an \egret~all-sky map at ${\rm E > 100 \mev}$ where the preponderance of unidentified sources and locations thereof are made clear. 

some more words to add here?


Despite the difficulties with definitively associating  low-latitude Galactic \egret ~sources with known counterparts, \cite{Sturner95} 
1. sturner says correlation with SNRs, yad says star forming regions, esposito does stats to suggest numbers
Despite being unable to make definitive associations 
...
EGRET statistical association with SNRs, but not concrete association

EGRET was not able to make definitive associations with SNRs, the LAT has the spatial and spectral resolution to do so.




\citep{Yadigaroglu97} \gam~sources correlate with massive star forming regions
\begin{figure}[h!]%[t] 
	\centering
