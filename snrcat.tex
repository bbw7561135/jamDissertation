...
There are several ways in which \ptlike{} improves in efficiency compared to the Science Tools. It saves computational time, while  sacrificing accuracy,
...
With its efficient likelihood calculations, \ptlike{} is ideally suited for large-scale studies, like all-sky analyses performed for the \lat{} point source catalogs, and analyses requiring several iterations, such as source finding/localization and TS map creation \jamie{assume I've mentioned TS maps already}). 
...
In this work we focus on the 279 currently known Galactic SNRs. They are derived from the $274$~SNRs noted in the catalog of \citet[hereafter Green's catalog]{Green09}, plus five additional SNRs identified following its publication. %\jack{Add link to table of SNRs here?} \jack{multiple tables... it's ok without link}
All but $16$ of these SNRs have been firmly identified by their radio synchrotron emission, so their centroids and extensions are primarily determined from the radio. % is often the primary means of determining. 
When the radio detection is not secure, positional information is obtained from the optical, X-ray, or TeV observations that identified the SNR. The catalog is thought to be complete down to a $1$\,GHz radio surface brightness limit of $\approx 10^{-20}$\,W\,m$^{-2}$\,Hz$^{-1}$\,sr$^{-1}$ (i.e.\,$1$\,MJy\,sr$^{-1}$). % ($\approx 10^{6}$\,Jy\,sr$^{-1}$). 
...
%(see Green 1991, 2004, 2005 for more detailed discussions).
% may cite Green 2012 review. Discusses selection effects in detail
...
\jamie{I need to describe the basic method and complications, intricacies  in the prevoois section, and then talk about snr cat specific things here}
