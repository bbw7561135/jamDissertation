...
By the end of the its science run, \egret{} had detected 271 sources above 100\mev{}, within a minimum detection significance of 4$\sigma$ \jamie{energy range of egret was 30 \mev{} to 20 \gev{}, but in the 3EG paper they don't say whether this was the actual max energy used. they do make maps $>1\gev $}, 170 of which had no clear multiwavelength counterpart, with 81 of those unidentified lying within \blat of the Galactic plane \citep{Hartman99}. The main hindrances to source identification were the numerous potential source counterparts (the \egret{} \psf{} was energy dependent, with a 68\% containment radius of ${\rm \sim 6 ^\circ}$ at 100\mev{} and smaller for higher energies) and the large \egret{} error boxes. In addition to this, the primary method for identifying a \gam{} source with as an
...
This bump in sensitivity results in the \lat{} detecting considerably more sources than \egret. \jamie{I'm not sure if this next part goes here or in the \lat{} chapter} Remarkably, within its first three months of commission, the \lat{} detected 205 sources (above {\rm 10$\sigma$ significance \citep{lat_3m}), and by 11 months, 1451 sources above 4$\sigma$ \citep{1FGL}, compared to  the aforementioned 271 over the entire \egret{} mission. In fact, over its lifetime, \egret{} detected a total of about ${\rm 1.5~x~10^6}$ cosmic photons, while as of March 2016, the \lat{} has detected ${\rm \sim 863~x~10^6}$ source class photons! 
...
difficulties, the majority of the analyses undertaken in this thesis are focused on the ${\rm E \geq 1\gev}$ energy range. This energy band is ideal for probing the properties of the accelerated particle populations present in the \snr{} environment. It's also an energy range that benefits from a finer \psf{}, striking a balance between minimizing the diffuse contribution, maximizing photon sensitivity, and retaining good photon statistics. Furthermore, evolved \snrs{} have been shown to exhibit a spectral break between 1-10\gev{}, ()with explanations for the break ranging from ), so focusing on this energy range allows us to something about looking for breaks \jamie{rephrase evolved? interecting}

Then say the lat can do it for all, and motivate snrcat and addSrcs

\section{\label{snrCat:catIntro}snrCat Inro}
\jamie{this isn't a real section, just figuring out how much of that text, if anything, to pull into the thesis}


\section{\label{snrCat:latGam}Supernova Remnants at \gam~Energies}

\jamie{this is all in intro right now, either get rid of this section or put that stuff in this section}
...
We have stats where EGRET didn't, above 1 GeV/10 GeV fermi vs egret

we can measure extension well above 1 GeV/ 10 GeV

There are interesting generating physics (emission mechanisms) that produce gamma-rays that don't occur at other energies, this energy range is one where we see spectral features, we have good stats, low bkg, pulsars cutoff
...
The lat operates at a crucial energy range for studying the \gam{} emission processes operating in \snrs. 
...
search for some spectral roll overs that occur in this energy range (exclusively for interacting snrs?
...
In this chapter, we describe our efforts towards detecting \gam~emission from coincident with entire population of known radio emitting \snrs{}
